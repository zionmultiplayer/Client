...
int SocketLayer::RecvFrom( const SOCKET s, RakPeer *rakPeer, int *errorCode )
{
	...
	{
	#if SAMP_SERVER
		if(*reinterpret_cast<uint32_t *>(data) == 'PMAS')
		{
			if(SAMP::HandleQuery)
				SAMP::HandleQuery(s, sa, (uint8_t *)data, len2);
			return 1;
		}

		uint8_t dbuffer[4096];
		SAMP::Decrypt(dbuffer, (uint8_t *)data, len);
	#endif

		unsigned short portnum;
		portnum = ntohs( sa.sin_port );
		#if SAMP_SERVER
			ProcessNetworkPacket( sa.sin_addr.s_addr, portnum, (char *)dbuffer, len - 1, rakPeer );
		#else
...
int SocketLayer::SendTo( SOCKET s, const char *data, int length, unsigned int binaryAddress, unsigned short port )
{
	...
	#if SAMP_CLIENT
		uint8_t ebuffer[4096];
		SAMP::Encrypt(ebuffer, (uint8_t *)data, length);
	#endif
	...
	do
	{
		...
		#if SAMP_CLIENT
			len = sendto( s, (char *)ebuffer, length + 1, 0, ( const sockaddr* ) & sa, sizeof( struct sockaddr_in ) );
		#else
